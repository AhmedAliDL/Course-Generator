...
class GenerateChat:
    ...
    # generate lectures based on course name and number of lectures
    def gen_lectures(self, lec_num: int, course_name: str):
        """this function just take the number of lectures that user
        want to make and take course`s name too , and then passing this
        information to the llm(gpt3.5) to produce the lectures based on
        specific prompt."""
        try:
            lectures = llm([
                SystemMessage(content="""You are course generator that generate courses based on number of lecture 
                and course name <!--start-example1-input--> number of lecture = {2},course name = {basic math} 
                <!--end-example1-input--> <!--start-example1-output--> Lecture 1: Introduction to Basic Math - 
                Overview of basic math concepts - Understanding numbers and number systems - Basic operations: 
                addition, subtraction, multiplication, and division - Introduction to fractions and decimals - 
                Practical applications of basic math in everyday life Lecture 2: Problem Solving and Equations - 
                Problem-solving strategies and techniques - Solving equations and inequalities - Introduction to 
                algebraic expressions - Understanding patterns and sequences - Practical problem-solving examples 
                <!--end-example1-output--> <!--start-input--> generate lectures <!--end-input--> 
                <!--start-output-->"""),
                HumanMessage(content=f"number of lecture = {lec_num},course name = {course_name}"),
            ]
            )
            text = lectures.content
            lec: dict = {}
            lec_name: str = ""
            lec_points = []
            for point in text.split("\n"):
                if point.__contains__("Lecture"):
                    if lec_points:
                        lec[lec_name] = lec_points
                        lec_points = []
                    lec_name = point.split(":")[0]
                    continue
                else:
                    if point:
                        lec_points.append(point)
            lec[lec_name] = lec_points
            return lec
    ...
    # generate slide of lecture
    def gen_slide_lec(self, text: str):
        """this function just take lectures`points that produced by gen_lectures
        function then by using llm(gpt3.5) generating slides based
        on specific prompt."""
        try:
            slides = llm(
                [
                    SystemMessage(
                        content=f"""Create a series of slides (pptx) based on the following text. Each slide should 
                        have a unique title and multi points that are derived from the provided text. Ensure that the 
                        presentation points are diverse and well-organized to effectively communicate the key aspects 
                        of the topic. The slides should be informative, engaging, and visually appealing, 
    ...
    # apply generating of lecture and slide
    def apply_gen_slide_lec(self, lec: dict):
        """this function take lectures(titles ,points) that generated
        by gen_lectures and then map this titles to the new points
        of slides generated by gen_slide_lec."""
        try:
            data: list = []
            for lec_n, lec_points in tqdm(lec.items()):
                tmp_container: list = []
                for point in lec_points:
                    if point == "":
                        continue
                    points = self.gen_slide_lec(point)
                    tmp_container.append(points)
                data.append(tmp_container)
            return data
